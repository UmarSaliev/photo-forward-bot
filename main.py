...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("/start — начать работу 💻/help — основные команды 📖/ping — проверить связь с хостом 🌐/status — статус активности бота 🤖/list — список учеников отправивших задания 🤓(доступен только учителю)/broadcast — рассылка сообщений ✈️(доступен только учителю)/task - 📌 Генерирует новую задачу по математике (на основе запроса ученика)./definition - 📘 Даёт определение математического термина./formula - 📐 Даёт формулу по указанной теме (например, площадь треугольника).")
